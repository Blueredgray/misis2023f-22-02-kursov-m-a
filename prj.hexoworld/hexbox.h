...
/*
struct smooth_trans {
    smooth_trans_args t1;
    smooth_trans_args t2;
    smooth_trans_args t3;
    smooth_trans_args t4;
    smooth_trans_args t5;
    smooth_trans_args t6;
};
*/
...
private:
    //std::vector <std::vector <int>>  hex_h;
    //std::vector <std::vector <hex_args>>  hex_grid; //массив шестиугольников

    //void set_number_ch(hex_args h_as, cint w);
    //int number_ch(const std::vector <hex_args>& h_g, cint x, cint y, cint w);
    ...
    void fe_hex_grid(cint x, cint y);
    void fe_st_grid(cint x, cint y);
...
/*
//вычисление номера шестиугольника по ширине поля (только если квадратное!)
void Hexbox::set_number_ch(hex_args h_as, cint w)
{
    h_as.number_ch = w * h_as.y + h_as.x;
}
*/
